...
describe('DOMParser', () => {
    describe('XML', () => {
        let xmlSingleLine = '<?xml version="1.0" encoding="UTF-8"?>' +
            '<note>' +
            '  <to>Tove</to>' +
            '  <from>Jani</Ffrom>' +
            ...
            '</note>';
        let xmlLinuxMultiLinesString = '<?xml version="1.0" encoding="UTF-8"?>\n' +
            '<note>\n' +
            '  <to>Tove</to>\n' +
            '  <from>Jani</Ffrom>\n' +
            '  <heading>Reminder</heading>\n' +
            "  <body>Don't forget me this weekend!</body>\n" +
            '</note>';
        let xmlWindowsMultiLinesString = '<?xml version="1.0" encoding="UTF-8"?>\r\n' +
            '<note>\r\n' +
            '  <to>Tove</to>\r\n' +
            '  <from>Jani</Ffrom>\r\n' +
            '  <heading>Reminder</heading>\r\n' +
        ...
        it('xml error single line content', (done) => {
            ...
            chai_1.expect(xmlDoc.documentElement.tagName === 'parsererror');
            chai_1.expect(XMLFeatures.getParserError(xmlDoc) != null);
            done();
        });
    ...
    describe('XML', () => {
        let htmlAttributWithoutValue = '<p data-modal-target>Hover over me for an inline modal!</p>' +
            '<div data-modal-content data-modal-align="right" data-modal-trigger="hover" data-modal-offset="10px">' +
            '</div>';
        let htmlEmptyTag = '<!DOCTYPE html><html><body><p>' +
...
//# sourceMappingURL=domParser.test..js.map